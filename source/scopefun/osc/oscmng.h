...
public:
    StringBuffer              debug;
public:
    EAutoCalibrateMessageBox messageBox;
...
public:
    ...
    void fftCalculate(uint samples, float* real, float* imag);
    void fftRCalculateAmp(uint samples, float* real, float* imag, float* outAmpl);
    void fftAmplitude(iint samples, float* outAmpl, float* inRe, float* inIm);
};
...
{
public:
    void Rectangle(double& rectW, double& rectH, double& rectX, double& rectY, uint width, uint height, double size);
    void MinMax(double& xMinimum, double& xMaximum, uint width, uint height, double viewX, double size, double zoom);
};
...
public:
    // controlThread
    int  openUSB(OscHardware* oscHw, bool factory);
    int  uploadFpga(OscHardware* oscHw);
    int  useEepromCalibration(OscHardware* oscHw);
    int  writeFpgaToArtix7(SHardware* ctrl, OscHardware* hw);

    int  writeUsbToEEPROM(OscHardware* hw, bool factory);
    int  readFirmwareIDFromEEPROM(OscHardware* hw);
    int  readUsbFromEEPROM(OscHardware* hw, int read = 0);
    int  writeCalibrateSettingsToEEPROM(OscHardware* hw);
    int  readCalibrateSettingsFromEEPROM(OscHardware* hw);
    int  eraseEEPROM(OscHardware* hw);
    int  hardwareControlFunction(SHardware* hw);
    int  eraseCalibrationEEPROM(OscHardware* hw);
    int  writeUsbToRam(OscHardware* hw, bool factory);
public:
    // captureThread
...
public:
    ...
    void onCalibrateFrameCaptured(SDisplay& frame, int version);
    void AutoCalibrate();
public:
