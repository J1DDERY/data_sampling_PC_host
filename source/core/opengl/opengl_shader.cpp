...
void CompileShader(const char* shaderName, GLuint& shader, const char* file, ilarge size, GrShaderDefine& def)
{
    ...
    //
    char*   macro[64] = {0};
    String  allShaderNames[64];
    String  string[64];

    std::string macroStr;
    std::array<std::string, 64> allShaderNamesStrArr;
    std::array<std::string, 64> stringStrArr;

    if(grGetMode() == OPENGL_MODE_21)
    {
        macro[0] = (char*)"#version 110\n";
        macroStr = "#version 110\n";
    }
    else
    {
        macro[0] = (char*)"#version 150 core\n";
        macroStr = "#version 150 core\n";
    }
    int a = 1;
    for(; a < def.define.getCount() + 1; a++)
    {
        string[a] = def.define.index(a - 1);
        stringStrArr[a] = def.define.index(a - 1);
        int defDefineCnt = def.define.getCount();
        if (string[a].getLength() < 2) {
            int j = 0;
            for (; j < defDefineCnt; j++) {
                allShaderNames[0].insert(0, def.define.index(j));
            }
            CORE_MESSAGE("\nWarning {a=%i}: shader name error (%s).\nFile: %s.\nStr. array cnt: %i\n%s", a, string[a].asChar(), shaderName, defDefineCnt, allShaderNames[0].asChar());
            break;
        }
