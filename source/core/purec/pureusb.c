...
int writeData(UsbContext* ctx, int address, char* buffer, int len)
{
    ...
    ret = libusb_control_transfer((libusb_device_handle*)ctx->device,
                                  ...
                                  len,
                                  10000);
    ret = memcmp((void*)buffer, (void*)verify, (long unsigned int)len);
    if(ret != 0)
    {
        int debug = 1;
    }
...
int usbFxxOpen(UsbContext* ctx, usbDevice* device)
{
    ...
    {
        printf("USB transfer speed reduced. Please ensure that ScopeFun is connected to USB 3.1 Gen1 compatible port.\n");
    }
    // interface
